...
## 권장 수집 일정 — 4 one-way trip (2일)

지하철 내부는 trip간 acoustic variation이 작아서 4 one-way로 충분 (sample-complexity 계산은 `CLAUDE.md` 참조).
...
| Day 2 | #3 | 등교 | train |
| Day 2 | #4 | 하교 | **test (격리, 증강 금지)** |
| Day 3 | (backup) | — | 위 4개 중 실패한 게 있으면 동시간대 재녹음 |

**트립 실패 정의** — 아래 한 가지라도 해당되면 재녹음:
- 마크 누락 6역 이상 (절반 이상 못 잡음)
- 슬라이스 후 wav가 잡음만 들리고 안내방송 안 들림 (LSB 분포 검사로 빠르게 진단)
- 평상시 RMS > 1000 / dBFS -10 위 (클리핑)

매 트립 출발 전 **사전 검증(섹션 2)** 통과 의무화하면 실패 거의 0.
