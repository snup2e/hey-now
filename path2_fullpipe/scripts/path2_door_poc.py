...
TRIPS = [1, 2, 3, 4, 5, 6, 7, 8]         # A_train audio (N).wav (8 trips as of 2026-06)
VARIANT_TRIP = None                      # chime-variant flag was for the old 4-trip set; stale at 8
...
def main():
    ...
    for k, name in (("dep", "출발 차임 이벤트"), ("state", "정차 상태 런"),
                    ("fuse", "융합(OR)")):
        h, n, f = tot[k]
        print(f"  LOO {name:14s}: {h}/{n} ({h/n*100:3.0f}%)  false {f} ({f/len(TRIPS):.1f}/trip)")
